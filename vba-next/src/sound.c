...
#include <math.h>
#ifdef ESP_PLATFORM
#include "esp_heap_caps.h"   /* blip buffers → PSRAM (free internal SRAM for the JIT arena) */
#endif
...
#ifndef VGBA_NOSOUND
#define VGBA_NOSOUND 0   /* 1 = skip all sound generation + blip writes (CPU win; also moots
                          * the JIT soundTicks/blip-overrun bug). Build: -DVGBA_NOSOUND=1 */
#endif

static INLINE void Blip_Synth_offset( const Blip_Synth *self, int32_t t, int delta, Blip_Buffer* buf )
{
#if VGBA_NOSOUND
	(void)self; (void)t; (void)delta; (void)buf; return;   /* no blip writes → no overrun */
#endif
	...
	{ int32_t _i = (int32_t)(time >> BLIP_BUFFER_ACCURACY);   /* safety: never write past the buffer */
	  if (_i < 0 || _i + 1 >= buf->buffer_size_) return; }
...
static const char * Blip_Buffer_set_sample_rate(Blip_Buffer *self, long new_rate, int msec)
{
   ...
   {
#ifdef ESP_PLATFORM
      /* Blip buffers are ~32KB each ×3 (~96KB). Keep them in PSRAM, NOT internal
       * SRAM: when the JIT holds its 60KB internal exec arena, the 3rd buffer
       * (bufs_buffer[0], allocated last) OOMs in internal DRAM → factor_/size stay
       * 0 → the stereo mixer's avail (read from buffer[0]) is always 0 → the center
       * buffer never drains → offset_ runs away → APU writes go OOB (broken audio).
       * PSRAM is fine here: blip mixing touches these once per frame, not per i-fetch. */
      void* p = heap_caps_realloc( self->buffer_, (new_size + BLIP_BUFFER_EXTRA_) * sizeof *self->buffer_,
...
{
#if VGBA_NOSOUND
	return;   /* skip all sound mixing (the per-sample blip synthesis) — core0 CPU win */
#endif
	long avail;
	int numSamples;
