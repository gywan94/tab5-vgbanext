...
#elif defined(ESP_PLATFORM) /* tab5: FreeRTOS task pinned to core1 */

#include <stdlib.h>
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"

/* The VBA renderer worker busy-spins on renderer_state (see threaded_renderer_loop).
 * Pin it to core1 at priority 4 — BELOW the vgba_run display task (priority 5) — so
 * the display task preempts the spin when a frame is ready to blit. Core0 stays free
 * for the CPU interpreter. (Core1 idle-WDT is disabled in sdkconfig so the busy-spin
 * doesn't trip it.) */
struct _esp_thr_args { threadfunc_t func; void *arg; };

static void _esp_thread_trampoline(void *p)
{
   struct _esp_thr_args *a = (struct _esp_thr_args *)p;
   threadfunc_t func = a->func;
   void        *arg  = a->arg;
   free(a);
   func(arg);            /* loops until renderer_control != 1 */
   vTaskDelete(NULL);    /* in case the worker function returns */
}

thread_t thread_run(threadfunc_t func, void *p, int priority)
{
   (void)priority;
   struct _esp_thr_args *a = (struct _esp_thr_args *)malloc(sizeof(*a));
   if (!a) return NULL;
   a->func = func; a->arg = p;
   TaskHandle_t h = NULL;
   if (xTaskCreatePinnedToCore(_esp_thread_trampoline, "vba_render",
                               16384, a, 4 /* < display task's 5 */, &h, 1 /* core1 */) != pdPASS) {
      free(a);
      return NULL;
   }
   return (thread_t)h;
...
void thread_sleep(int ms) { vTaskDelay(pdMS_TO_TICKS(ms ? ms : 1)); }
void thread_set_priority(thread_t id, int priority) { (void)id; (void)priority; }
