...
/* tab5 ROM-buffer sizing. Upstream allocates a flat 32 MB ROM buffer; that does
 * not fit in the M5Tab5's 32 MB PSRAM alongside framebuffers/work RAM. Instead we
 * size the buffer to the actual ROM rounded up to a power of two (+ headroom for
 * the HLE-BIOS SWI trampoline). g_rom_mask wraps every ROM access inside the
 * smaller buffer; g_swi_base is where the trampoline (normally at the fixed
 * offset 0x1fe2000) is relocated, and the two high-ROM literals baked into
 * myROM[] are patched to match in CPUInit(). Defaults = the original 32 MB
 * behaviour until CPULoadRomData() recomputes them. */
uint32_t g_rom_alloc = 0x2000000u;
uint32_t g_rom_mask  = 0x01FFFFFFu;
uint32_t g_swi_base  = 0x01FE2000u;
...
static INLINE uint32_t CPUReadMemory(uint32_t address)
{
	...
	switch(address >> 24)
	{
		...
		case 0x0C:
			/* gamepak ROM */
			value = READ32LE(rom + (address & (g_rom_mask & ~3u)));
			break;
		case 0x0D:
         value = eepromRead();
         break;
	...
	switch(address >> 24)
	{
		...
		case 12:
			...
			case 0x80000c8:
				...
				break;
			default:
				value = READ16LE(rom + (address & (g_rom_mask & ~1u))); break;
			}
			break;
		case 13:
         value =  eepromRead();
...
/* tab5 dynarec hook (defined in the vgba_jit component; resolved at final app
 * link, no component REQUIRES edge — avoids a vbanext<->vgba_jit cycle). */
extern int g_vgba_jit;
extern int vgba_jit_thumb_dispatch(uint32_t pc);
extern void vgba_jit_profile(uint32_t pc);     /* g_vgba_jit==2: measurement-only */
extern void *vgba_jit_get_block(uint32_t pc, int *out_ninstr);  /* M6 chaining builder */
extern unsigned g_vgba_jit_instrs;             /* stats: THUMB instrs run via JIT */
extern void vgba_jit_set_state(uint32_t *reg, uint8_t *n, uint8_t *c, uint8_t *z, uint8_t *v,
                               uint8_t *rom, uint32_t rom_mask, uint8_t *iwram, uint8_t *ewram);
extern void vgba_jit_set_helpers(void *ld32, void *ld16, void *ld16s, void *ld8, void *ld8s,
...
static int HOT_IRAM thumbExecute (void)
{
   ...
   do
   {
      ...
      /* tab5 dynarec (M4): the THUMB instruction about to execute is at
       * bus.armNextPC (reg15 = armNextPC+2, see the branch convention). If a
       * native block exists/can be built there (main ROM 0x08-0x09), run its
       * `njit` consecutive simple ops (data effects only), then advance the
       * pipeline ONCE to "about to execute armNextPC + 2*njit" using the same
       * post-branch convention (armNextPC=target, reg15=target+2, prefetch
       * refilled). Simple ROM ops are 1S → njit*(seq16+1) cycles. O(1) per block. */
      /* JIT only ROM (0x08-0x0D): read-only → SMC-safe. EWRAM(0x02)/IWRAM(0x03)
       * are writable; JIT-ing them needs SMC invalidation (later step) — without
       * it, games overwriting code there leave stale blocks → corruption/crash. */
      /* tab5 dynarec (M6): when armed (g_vgba_jit==1) and the THUMB instruction
       * about to execute (at bus.armNextPC) is in main ROM (0x08-0x0D, read-only
       * → SMC-safe), run NATIVE blocks back-to-back. Each block applies straight-
       * line data effects then a branch terminator and RETURNS the next THUMB PC
       * (a0); we chain to it until execution leaves ROM, hits a non-translatable
       * entry, or reaches a cpu event boundary — then resync the interpreter's
       * pipeline ONCE (armNextPC/reg15/cpuPrefetch). Dispatch is per-BASIC-BLOCK,
       * not per-instruction. g_vgba_jit==2 = measurement-only profiler. */
      {
      uint32_t _r = bus.armNextPC >> 24;
      if (g_vgba_jit == 2) {
         ...
         for (;;) {
            int _ninstr;
            uint32_t _start = _bpc;
            void *_code = vgba_jit_get_block(_start, &_ninstr);
            if (!_code) break;                              /* entry not translatable → interpreter */
#ifndef VGBA_JIT_VALIDATE
#define VGBA_JIT_VALIDATE 0   /* dev: run block AND interpreter on same state, compare, keep interp result */
#endif
#if VGBA_JIT_VALIDATE
            {
               extern int printf(const char *, ...);
               static int _vmax = 0;
               uint32_t _s0[18]; for (int _i = 0; _i < 18; _i++) _s0[_i] = bus.reg[_i].I;
               bool _sN = N_FLAG, _sZ = Z_FLAG, _sC = C_FLAG, _sV = V_FLAG;
               ((uint32_t (*)(void))_code)();                /* run JIT block (mutates reg/flags) */
               uint32_t _jr[18]; for (int _i = 0; _i < 18; _i++) _jr[_i] = bus.reg[_i].I;
               bool _jN = N_FLAG, _jZ = Z_FLAG, _jC = C_FLAG, _jV = V_FLAG;
               for (int _i = 0; _i < 18; _i++) bus.reg[_i].I = _s0[_i];  /* restore pre-state */
               N_FLAG = _sN; Z_FLAG = _sZ; C_FLAG = _sC; V_FLAG = _sV;
               uint32_t _ip = _start;                        /* re-run same ops via VBA interpreter */
               for (int _k = 0; _k < _ninstr; _k++) { uint16_t _o = CPUReadHalfWordQuick(_ip); (*thumbInsnTable[_o >> 6])(_o); _ip += 2; }
               if (_vmax < 30) {
                  uint16_t _op0 = CPUReadHalfWordQuick(_start);
                  for (int _i = 0; _i < 16; _i++) if (bus.reg[_i].I != _jr[_i]) {
                     printf("JITVAL @%08lx op=%04x r%d jit=%08lx ref=%08lx (in r0=%08lx r1=%08lx)\n",
                            (unsigned long)_start, _op0, _i, (unsigned long)_jr[_i], (unsigned long)bus.reg[_i].I,
                            (unsigned long)_s0[(_op0>>3)&7], (unsigned long)_s0[_op0&7]); _vmax++; break; }
                  if (N_FLAG != _jN || Z_FLAG != _jZ || C_FLAG != _jC || V_FLAG != _jV) {
                     printf("JITVAL @%08lx op=%04x FLAG jitNZCV=%d%d%d%d refNZCV=%d%d%d%d\n",
                            (unsigned long)_start, _op0, _jN,_jZ,_jC,_jV, N_FLAG,Z_FLAG,C_FLAG,V_FLAG); _vmax++; }
               }
               _bpc = _ip;                                   /* interpreter result authoritative */
            }
#else
            _bpc = ((uint32_t (*)(void))_code)();           /* run block; a0 = next PC */
#endif
            /* Cycle accounting: codeTicksAccessSeq16() returns 0 once the gamepak prefetch
             * (busPrefetchCount) fills, which the JIT never builds — so calling it charges
             * every ROM op the full waitstate (~3×) → cpuTotalTicks overcounts → soundTicks
             * races negative → t = SOUND_CLOCK_TICKS - soundTicks explodes → blip overrun.
             * With prefetch on (koudai), sequential ROM ops cost ~1; approximate that. */
            cpuTotalTicks += (uint32_t)_ninstr;   /* (×3 diag proved blip runaway is INDEPENDENT of cycle count) */
            g_vgba_jit_instrs += (unsigned)_ninstr;
            _ran = 1;
            uint32_t _nr = _bpc >> 24;
            if (_nr < 0x08 || _nr > 0x0D) break;            /* left ROM region */
            if (cpuTotalTicks >= cpuNextEvent) break;       /* cpu event boundary */
         }
         if (_ran) {
            bus.armNextPC   = _bpc;
            bus.reg[15].I   = _bpc + 2;
            cpuPrefetch[0]  = CPUReadHalfWordQuick(_bpc);
            cpuPrefetch[1]  = CPUReadHalfWordQuick(_bpc + 2);
            bus.busPrefetch = false;
            test = cpuTotalTicks < cpuNextEvent && !armState && !holdState;
            if (test) continue; else break;
         }
...
static bool CPUSetupBuffers(void)
{
	...
	rom = (uint8_t *)memalign_alloc_aligned(g_rom_alloc);
...
static void CPULoadRomGeneric(uint8_t *whereToLoad)
{
   ...
   uint16_t *temp;
	...
	for(i = (romSize+1)&~1; i < (int)g_rom_alloc; i+=2)
   {
...
int CPULoadRomData(const char *data, int size)
{
   uint8_t *whereToLoad;
	...
	/* tab5: size the ROM buffer to this ROM (power of two) + trampoline headroom
	 * instead of a flat 32 MB, so it fits in PSRAM. Must run BEFORE CPUSetupBuffers
	 * (which allocates rom using g_rom_alloc). */
	{
		uint32_t need = (uint32_t)size + 0x20000u;   /* ROM + > 0x1E000 trampoline room */
		if (need > 0x2000000u) need = 0x2000000u;    /* GBA 32 MB cap; also stops the pow2 loop below from overflowing on a bogus huge size */
		uint32_t n = 0x8000u;
		while (n < need) n <<= 1;                     /* n in [0x8000, 0x2000000] */
		g_rom_alloc = n;
		g_rom_mask  = n - 1u;
		g_swi_base  = n - 0x1E000u;
	}
	...
	whereToLoad = rom;
	if ((uint32_t)size > g_rom_alloc) size = (int)g_rom_alloc;  /* never overrun the (capped) ROM buffer */
...
/* tab5 dynarec memory helpers — the JIT emits CALLs to these because the real
 * CPUReadMemory/CPUWriteMemory are static INLINE (not linkable). They carry
 * VBA's exact per-region semantics + side effects (I/O, VRAM, etc.). */
uint32_t vgba_h_ld32 (uint32_t a)            { return CPUReadMemory(a); }
uint32_t vgba_h_ld16 (uint32_t a)            { return CPUReadHalfWord(a); }
uint32_t vgba_h_ld16s(uint32_t a)            { return (uint32_t)(int32_t)(int16_t)CPUReadHalfWordSigned(a); }
uint32_t vgba_h_ld8  (uint32_t a)            { return CPUReadByte(a); }
uint32_t vgba_h_ld8s (uint32_t a)            { return (uint32_t)(int32_t)(int8_t)CPUReadByte(a); }
void     vgba_h_st32 (uint32_t a, uint32_t v){ CPUWriteMemory(a, v); }
void     vgba_h_st16 (uint32_t a, uint32_t v){ CPUWriteHalfWord(a, (uint16_t)v); }
void     vgba_h_st8  (uint32_t a, uint32_t v){ CPUWriteByte(a, (uint8_t)v); }
...
void CPUInit(const char *biosFileName, bool useBiosFile)
{
	...
	if(!cpuBiosSwapped)
   {
	...
	/* tab5 dynarec (M2): register the addresses of our file-static CPU state with
	 * the JIT so the emitter can bake them into generated blocks. reg_pair is 4
	 * bytes, so &bus.reg[0].I is a uint32_t[] of the GPRs (reg[15]=PC); flags are
	 * 1-byte bools. */
	vgba_jit_set_state(&bus.reg[0].I, (uint8_t *)&N_FLAG, (uint8_t *)&C_FLAG,
	                   (uint8_t *)&Z_FLAG, (uint8_t *)&V_FLAG, rom, g_rom_mask,
	                   internalRAM, workRAM);
	vgba_jit_set_helpers((void*)vgba_h_ld32, (void*)vgba_h_ld16, (void*)vgba_h_ld16s,
	                     (void*)vgba_h_ld8, (void*)vgba_h_ld8s,
	                     (void*)vgba_h_st32, (void*)vgba_h_st16, (void*)vgba_h_st8);
	...
	/* tab5: when the ROM buffer is shrunk below 32 MB (see CPULoadRomData), the
	 * HLE BIOS's two high-ROM literals must point at the relocated trampoline
	 * region instead of the fixed 0x09FE2000 / 0x09FFC000. No-op in the 32 MB
	 * case (new values equal the originals). Only patch the HLE BIOS image. */
	if(!useBios) {
		uint32_t new0 = 0x08000000u + g_swi_base;            /* was 0x09FE2000 (off 0x1fe2000) */
		uint32_t new1 = 0x08000000u + g_swi_base + 0x1A000u; /* was 0x09FFC000 (off 0x1ffc000) */
		int j;
		for(j = 0; j + 4 <= (int)sizeof(myROM); j += 4) {
			uint32_t w = READ32LE(&bios[j]);
			if(w == 0x09FE2000u)      WRITE32LE(&bios[j], new0);
			else if(w == 0x09FFC000u) WRITE32LE(&bios[j], new1);
		}
	}
	...
	if(romSize < g_swi_base) {
		WRITE16LE(&rom[g_swi_base + 0x9c], 0xdffa); /* SWI 0xFA */
		WRITE16LE(&rom[g_swi_base + 0x9e], 0x4770); /* BX LR */
	}
...
void CPUReset (void)
{
	...
	map[7].mask = 0x3FF;
	map[8].address = rom;
	map[8].mask = g_rom_mask;
	map[9].address = rom;
	map[9].mask = g_rom_mask;
	map[10].address = rom;
	map[10].mask = g_rom_mask;
	map[12].address = rom;
	map[12].mask = g_rom_mask;
...
#define CHEAT_PATCH_ROM_16BIT(a,v) \
  WRITE16LE(((uint16_t *)&rom[(a) & g_rom_mask]), v);

#define CHEAT_PATCH_ROM_32BIT(a,v) \
  WRITE32LE(((uint32_t *)&rom[(a) & g_rom_mask]), v);
